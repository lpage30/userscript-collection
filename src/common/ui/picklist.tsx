...
export function PickList<T = string>({
    ...
    const [selectedOptions, setSelectedOptions] = useState<Array<PickOption<T>>>(Array.isArray(value) ? value : value ? [value] : [])
    showSelectAll = showSelectAll ?? multiple
    useEffect(() => {
        if(onChange) {
            const selection = [...selectedOptions.map(opt => opt.value)]
    ...
    const onSelectOptions = (option: PickOption<T> | PickOption<T>[], select: boolean): void => {
        if (Array.isArray(option)) {
            setSelectedOptions(select ? option : [])
            return
        }
    ...
    const isSelectAll = (): boolean => selectedOptions.length == options.length
    const isSelected = (option: PickOption<T>) => {
        return isSelectAll() ? true : selectedOptions.some((opt) => opt.label === option.label)
    }
    const getMaxPlaceholderText = (): string => {
        const opts: PickOption<T>[] = [...options].sort((l, r) => r.label.length - l.label.length)
        const maxselectDisplay = opts.slice(0, selectedDisplayMax ?? (selectedOptions.length - 1)).map(({label}) => label)
        const maxString =  [
    ...
    const getPlaceholderText = (): string => {
        if (isSelectAll() || (selectedDisplayMax && selectedDisplayMax <= selectedOptions.length)) {
            return `${selectedOptions.length}${isSelectAll() ? ' (All) ': ' '}items selected`
        }
